...
nbCycles = 1e4
print('Training with '..nbCycles..' cycles ...')
for i = 1, nbCycles do
   ...
   --logger:plot()  
   if i == nbCycles then print('Loss is ' .. current_loss) end
end
print('')
...
function least_squares(x, y)
   local xb = torch.cat(torch.ones((#x)[1]), x, 2)
   return torch.inverse(xb:t() * xb) * xb:t() * y
end

print('Least Squares:')
local ls = least_squares(data[{ {}, {2,3} }], data[{ {}, {1} }])
print(ls)
