...
for i = 1, iterations do
  ...
  if i % 10 == 0 then -- don't print *every* iteration, this is enough to get the gist
      ...
      --tlosses[#tlosses + 1] = criterion:forward(model:forward(test.data), test.labels)
  end
...
gnuplot.plot({
      ...
      '-'}, {
      torch.linspace(1, #losses, #tlosses),
      torch.Tensor(tlosses),
      '-'
})
]]--
------------------------------------------------------------------------------
...
print('last batch loss: '.. losses[#losses])
--print('last test loss: '.. tlosses[#tlosses])

local err = torch.csub(test.labels:long(), classPredictions):nonzero():size(1) / test.labels:size(1)
print('error: ' .. err)
